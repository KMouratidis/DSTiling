...
export default class Snap {
  ...
  _neighbor(window, direction) {
    return global.display.get_monitor_neighbor_index(window.get_monitor(), direction);
  }
  ...
  snapLeft() {
    ...
    if (side === "left") {
      ...
      this._snapTo(w, "right", Main.layoutManager.getWorkAreaForMonitor(n));
  ...
  snapRight() {
    ...
    if (side === "right") {
      ...
      this._snapTo(w, "left", Main.layoutManager.getWorkAreaForMonitor(n));
  ...
  toggleMaximize() {
    ...
    if (!this._snapped.has(w) && !this._thirds.has(w)) this._captureIfAbsent(w); // free -> remember size
