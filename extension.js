...
export default class DSTilingExtension extends Extension {
  enable() {
    ...
    this._connect(global.display, "grab-op-begin", (d, w, op) => this._snap.onGrabBegin(d, w, op));
    this._connect(global.display, "grab-op-end", (d, w, op) => this._snap.onGrabEnd(d, w, op));

    // Keep snapped halves aligned when the work area changes (panel/dock).
    this._connect(global.display, "workareas-changed", () => this._snap.refitAll());
